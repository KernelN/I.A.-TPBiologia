...
public class Move : MonoBehaviour
{
    ...
    {
        if (other.name == "Top" || other.name == "Left Wall" || other.name == "Right Wall" || other.name == "Floor")
        {
            EndMove();
            StartLeftRotation();
        }
    ...
    void Update()
    {
        ...
        transform.position += (transform.up * speed * Time.deltaTime / gameObject.GetComponent<Energy>().BodySize);
        transform.Rotate(0, 0, rotateSpeed * Time.deltaTime / gameObject.GetComponent<Energy>().BodySize);
        FreezeExtras();

        GetComponent<Energy>().energy -= (MoveTotal + (RotateTotal) / 500);
    }
}
